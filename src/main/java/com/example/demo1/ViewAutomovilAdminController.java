...
public class ViewAutomovilAdminController {
    ...
    @FXML
    private TableColumn<ObservableList<String>, String> clmPlacaVehi;

    @FXML
    private TableColumn<ObservableList<String>, Integer> clmTipoVeh;

    private ViewAgregarVehiculoController ventanaPrincipalController;

    public void setVentanaPrincipalController(ViewAgregarVehiculoController controller) {
    ...
    private void initialize() {
        ...
        clmPlacaVehi.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().get(6)));
        clmTipoVeh.setCellValueFactory(new Callback<TableColumn.CellDataFeatures<ObservableList<String>, Integer>, ObservableValue<Integer>>() {
            @Override
            public ObservableValue<Integer> call(TableColumn.CellDataFeatures<ObservableList<String>, Integer> param) {
                return new SimpleIntegerProperty(Integer.parseInt(param.getValue().get(7))).asObject();
            }
        });
        ...
        btnEliminar.setOnAction(event -> {
            ObservableList<String> selectedItem = tblVehiculos.getSelectionModel().getSelectedItem();
    ...
    void abrirVentanaAgregar(ActionEvent event) {
        try {
            ...
            FXMLLoader loader = new FXMLLoader(getClass().getResource("ViewAgregarVehiculo.fxml"));
            Parent root = loader.load();

            ViewAgregarVehiculoController agregarVehiculoController = loader.getController();
            agregarVehiculoController.setVentanaPrincipalController(this);
    ...
    @FXML
    void abrirVentanaEditarVehiculo(ActionEvent event) {
        ObservableList<String> vehiculoSeleccionado = tblVehiculos.getSelectionModel().getSelectedItem();

        if (vehiculoSeleccionado != null) {
            try {
                // Cargar la nueva ventana de edición
                FXMLLoader loader = new FXMLLoader(getClass().getResource("ViewEditarVehiculo.fxml"));
                Parent root = loader.load();

                // Obtener el controlador de la ventana de edición
                ViewEditarVehiculoController editarController = loader.getController();

                // Obtener datos del vehículo seleccionado y pasarlos al controlador de la ventana de edición
                String id = vehiculoSeleccionado.get(0);
                String modelo = vehiculoSeleccionado.get(1);
                String precio = vehiculoSeleccionado.get(2);
                String marcaId = vehiculoSeleccionado.get(3);
                String kilometraje = vehiculoSeleccionado.get(4);
                String gamaId = vehiculoSeleccionado.get(5);
                String placa = vehiculoSeleccionado.get(6);
                String tipo = vehiculoSeleccionado.get(7);


                editarController.inicializarDatos(id, modelo, precio, marcaId, kilometraje, gamaId, placa, tipo, this);
    ...
    public void cargarTodosLosVehiculos() {
        try (Connection connection = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:XE", "BRITO", "123456789")) {
            String query = "SELECT A.ID, A.MODELO, A.PRECIO, M.NOMBRE AS NOM_MARCA, A.KILOMETRAJE, G.DESCRIPCION, A.PLACA, A.TIPO_AUTOMOVIL_ID FROM AUTOMOVIL A JOIN MARCA M ON M.ID = A.MARCA_ID JOIN GAMA G ON A.GAMA_ID = G.ID";
            try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
                try (ResultSet resultSet = preparedStatement.executeQuery()) {
                    ...
                    while (resultSet.next()) {
                        ...
                        fila.add(resultSet.getString("TIPO_AUTOMOVIL_ID"));
                        hoteles.add(fila);

                        // Agregar impresión de valores
